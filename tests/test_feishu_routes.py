...
def test_feishu_error_after_create_does_not_undo_local_save(
    ...
    def fail_sync(db, achievement):
        sync_calls.append(achievement.id)
        raise FeishuNetworkError("private upstream response")
...
def test_feishu_error_after_update_does_not_undo_local_changes(
    ...
    assert sync_calls == [achievement_id]
    db = SessionLocal()
    try:
...
def test_feishu_sync_post_route_is_registered_before_dynamic_detail_route(app):
    matching_routes = [
        route
        for route in app.routes
        if getattr(route, "path", None)
        == "/achievements/{achievement_id}/feishu-sync"
    ]

    assert len(matching_routes) == 1
    assert matching_routes[0].methods == {"POST"}
    route_paths = [getattr(route, "path", "") for route in app.routes]
    assert route_paths.index("/achievements/{achievement_id}/feishu-sync") < (
        route_paths.index("/achievements/{achievement_id}")
    )
